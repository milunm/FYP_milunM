import os
from pathlib import Path #better file paths

import torch #main pytorch package
import torch.nn as nn #neural network module
from torch.utils.data import DataLoader #data loader class to make batches
from torchvision.utils import save_image #utility to save images and verify outputs

from src.dataset import PairedDataset #import my paired dataset class that pairs tensors

from src.models.discriminator import Discriminator #import Discriminator model

from src.models.unet_model import UNet #import UNet model



def save_debug_grid(x, pred, y, out_path: Path): #function to save debug image grids
  
    out_path.parent.mkdir(parents=True, exist_ok=True) #will create parent directories if they don't exist

    pred = pred.clamp(-1, 1) #clamp predictions to [-1, 1] range, matching dataset normalization

    # denorm [-1,1] -> [0,1] for image display, to avoid dark and unnatural images 
    x_vis = (x + 1) / 2
    y_vis = (y + 1) / 2
    p_vis = (pred + 1) / 2

    # sketch is 1-channel -> repeat to 3 channels for viewing
    x_vis = x_vis.repeat(1, 3, 1, 1)

    grid = torch.cat([x_vis, p_vis, y_vis], dim=0) #stack into one big batch: [inputs, predicted,targets]
    save_image(grid.clamp(0, 1), str(out_path), nrow=x.size(0)) #save as image grid


@torch.no_grad() #disable gradient calculations for evaluation
def evaluate_epoch(model, loader, loss_fn, device): #inputs model, data loader, loss function, device
    
    #Runs evaluation on a loader (val or test).
    #Returns average loss over all batches.
    
    model.eval() #set model to evaluation mode
    total = 0.0 #initialize total loss to 0

    for x, y in loader: #for each batch in the data loader
        x = x.to(device)
        y = y.to(device) #move input and target batches to device (cpu or gpu)

        pred = model(x) #forward pass: get model predictions
        loss = loss_fn(pred, y) #compute loss between predictions and ground truth
        total += loss.item() #accumulate loss

    return total / max(1, len(loader)) #return average loss over all batches


def train_GAN( #inputs for GAN training function
    
    run_name: str, #name for this training run (used for saving checkpoints)
    epochs: int = 10, #number of training epochs CAN B easily changed
    lr: float = 2e-4, #learning rate for optimizer
    batch_size: int = 8, #batch size for data loader
    size: int = 256, #image size
    num_workers: int = 0, #number of worker threads for data loading (set to 0 as issues occured with higher numbers on Windows)
    pin_memory: bool = True, #pin memory for faster data transfer to GPU
    save_debug_every: int = 5,#save debug grid every N epochs
    
):
    
    #GAN trainer for sketch->color models.
    #- trains on train split
    #- runs validation each epoch
    
    
    

    # device prioritization: GPU if available, else CPU
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device:", device) #print which device is being used
    Generator = UNet(in_channels=1, out_channels=3)  # UNet model is the Generator, with 1 input channel (L) and 3 output channels (RGB)
    Discrim = Discriminator(in_channels=4)  # Discriminator model with 4 input channels (L + RGB)

    # -------- data --------
    train_ds = PairedDataset(split="train", size=size) #create dataset object pointing at training input and target folders
    val_ds = PairedDataset(split="val", size=size) #create dataset object pointing at validation input and target folders
    

    train_loader = DataLoader( #data loader for training batches and shuffle data
        train_ds,
        batch_size=batch_size,
        shuffle=True,
        num_workers=num_workers,
        pin_memory=pin_memory
    )

    val_loader = DataLoader( #data loader for validation batches without shuffling
        val_ds,
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        pin_memory=pin_memory
    )

    

    # -------- model --------
    Generator = Generator.to(device) #move Generator model to the selected device (GPU/CPU)
    Discrim = Discrim.to(device) #move Discriminator model to the selected device (

    # -------- loss + optimizers --------
    bceLoss = nn.BCEWithLogitsLoss() #binary cross entropy loss with logits for stability
    l1Loss = nn.L1Loss() #L1 loss for pixel-wise difference

    generator_opt = torch.optim.Adam(Generator.parameters(), lr=lr, betas=(0.5, 0.999)) #Adam optimizer for Generator
    discriminator_opt = torch.optim.Adam(Discrim.parameters(), lr=lr, betas=(0.5, 0.999)) #Adam optimizer for Discriminator

    # -------- checkpoint folder --------
    ckpt_dir = Path("src/models/checkpoints") / run_name #create checkpoint directory path with run name
    ckpt_dir.mkdir(parents=True, exist_ok=True) #create the checkpoint directory if it doesn't exist

    best_val = float("inf") #initialize best validation loss to infinity

    for epoch in range(1, epochs + 1): #training loop for specified number of epochs
        # ===== TRAIN =====
        
        Discrim.train()           
        train_total = 0.0
        l1_total = 0.0
        for real_grayscale, real_rgb in train_loader: #for each batch in the training data loader
            real_grayscale = real_grayscale.to(device)
            real_rgb = real_rgb.to(device) #move input and target batches to device (cpu or gpu)

            #train Discriminator
            with torch.no_grad():
                fake_rgb = Generator(real_grayscale) #generate fake RGB images from grayscale inputs with Generator (nograd as Discriminator training only
            
            real_pair = torch.cat([real_grayscale, real_rgb], dim=1) #concatenate grayscale and real RGB images along channel dimension
            fake_pair = torch.cat([real_grayscale, fake_rgb], dim=1) #concatenate grayscale and fake RGB images along channel dimension 

            predict_real = Discrim(real_pair) #Discriminator prediction on real image pairs
            predict_fake = Discrim(fake_pair) #Discriminator prediction on fake image pairs

            discriminator_real_loss = bceLoss(predict_real, torch.ones_like(predict_real)) #loss for real images (should predict 1) measures difference between predicted and true labels
            discriminator_fake_loss = bceLoss(predict_fake, torch.zeros_like(predict_fake)) #loss for fake images (should predict 0)
            discriminator_loss = (discriminator_real_loss + discriminator_fake_loss) / 2 #average discriminator loss for fake and real
            discriminator_opt.zero_grad() #zero gradients for Discriminator after training step
            discriminator_loss.backward() #backpropagate discriminator loss
            discriminator_opt.step() #update Discriminator weights

            #train Generator
            Generator.train()
            fake_rgb = Generator(real_grayscale) #generate fake RGB images from grayscale inputs with Generator
            fake_pair = torch.cat([real_grayscale, fake_rgb], dim=1) #concatenate grayscale and fake RGB images along channel dimension
            predict_fake = Discrim(fake_pair) #Discriminator prediction on fake image pairs
            adversarial_loss = bceLoss(predict_fake, torch.ones_like(predict_fake)) #Generator wants Discriminator to predict 1 for fake images, calculate adversarial loss
            l1_loss = l1Loss(fake_rgb, real_rgb) #L1 loss between generated and real RGB images
            generator_loss = adversarial_loss + (100 * l1_loss) #total Generator loss is adversarial + weighted L1 loss
            generator_opt.zero_grad() #zero gradients for Generator after training step
            generator_loss.backward() #backpropagate generator loss
            generator_opt.step() #update Generator weights

            train_total += generator_loss.item() #accumulate training loss
            l1_total += l1_loss.item() #accumulate L1 loss
        
        train_avg = train_total / max(1, len(train_loader)) #compute average training loss over all batches
        l1_avg = l1_total / max(1, len(train_loader)) #compute average L1 loss over all batches

        print(f"Epoch {epoch}: Train Loss = {train_avg:.4f}, L1 Loss = {l1_avg:.4f}")

        #Validate every epoch using above defined function. also prints val loss
        val_avg = evaluate_epoch(Generator, val_loader, l1Loss, device) #compute average validation loss over all batches
        print(f"Epoch {epoch}: Val L1 Loss = {val_avg:.4f}")

        #save checkpoints
        torch.save( 
            {
                "epoch": epoch,
                "generator_state": Generator.state_dict(),
                "discriminator_state": Discrim.state_dict(),
                "generator_opt_state": generator_opt.state_dict(),
                "discriminator_opt_state": discriminator_opt.state_dict(),
                "train_loss": train_avg,
                "val_l1": val_avg,
            },
            ckpt_dir / "last_gan.pt" #save last checkpoint
        )
        #  SAVE best checkpoint (based on val)
        if val_avg < best_val:
            best_val = val_avg
            torch.save(
                {
                    "epoch": epoch,
                    "generator_state": Generator.state_dict(),
                    "discriminator_state": Discrim.state_dict(),
                    "generator_opt_state": generator_opt.state_dict(),
                    "discriminator_opt_state": discriminator_opt.state_dict(),
                    "best_val_l1": best_val,
                },
                ckpt_dir / "best_gan.pt" #save best checkpoint
            )
            print(" saved best_gan.pt")

        # SAVE debug images depending on inputted frequency


        

        if epoch == 1 or (save_debug_every and epoch % save_debug_every == 0): #every N epochs as inputted
            Generator.eval()
            x_vis, y_vis = next(iter(val_loader))
            x_vis = x_vis.to(device)
            y_vis = y_vis.to(device) #get a single batch from the validation loader and move to device

            with torch.no_grad():
                pred_vis = Generator(x_vis) #get model predictions for this batch, without gradient calculations as it's eval

            save_debug_grid( # save debug image grid function defined above
                x_vis, pred_vis, y_vis,
                ckpt_dir / f"debug_epoch{epoch:02d}.png"
            )
            print(f"Saved debug_epoch{epoch:02d}.png")

    print(f"Done. Checkpoints saved in: {ckpt_dir}") #final print statement with checkpoint directory
